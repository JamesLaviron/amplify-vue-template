  ...
  try {
    ...
    if (profiles.length === 0) {
      ...
      const { data: newProfile } = await client.models.UserProfile.create({
        username: props.user.username || props.user.signInDetails?.loginId?.split('@')[0] || 'Player',
    ...
    const { data: teams } = await client.models.FantasyTeam.list({
      filter: { ownerId: { eq: userProfile.value.id } }
    })
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          ...
          <Card>
            <CardContent class="p-4 text-center">
              ...
              <div class="text-2xl font-bold text-primary">{{ userProfile?.currentGameweek || 1 }}</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent class="p-4 text-center">
              <div class="text-sm text-muted-foreground mb-2">Budget</div>
              <div class="text-2xl font-bold text-primary">£{{ fantasyTeam?.budget?.toFixed(1) || '100.0' }}m</div>
            </CardContent>
          </Card>
        </div>
