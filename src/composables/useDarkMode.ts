...
export function useDarkMode() {
  ...
  const getStorageKey = () => {
    return currentUser.value ? `theme_${currentUser.value.userId || currentUser.value.username}` : 'theme_guest'
  }
  ...
  const getFromStorage = (): 'dark' | 'light' | null => {
    ...
    if (!theme) {
      ...
      theme = (sessionStorage.getItem('theme') || localStorage.getItem('theme')) as 'dark' | 'light' | null
    }
  ...
  onMounted(() => {
    ...
    const checkAuthState = async () => {
      try {
        ...
        if (user && (user.userId || user.username) !== (currentUser.value?.userId || currentUser.value?.username)) {
